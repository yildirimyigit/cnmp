...
# (X, Y, gamma) = sample((X, Y, gamma), num=30)
# (v_X, v_Y, v_gamma) = sample((v_X, v_Y, v_gamma), num=8)
...
test_X, test_Y, test_gamma = X[0].reshape(1, time_len, d_x), Y[0].reshape(1, time_len, d_x), \
                             gamma[0].reshape(1, time_len, d_x)
# ###############################################
    ...
    if plot:  # We highly recommend that you customize your own plot function, but you can use this function as default
        if final:
            for i in range(d_y):  # for every feature in Y vector we are plotting training data and its prediction
                ...
                plt.plot(time_range, test_Y[0, :, i], color='red')
...
conditioning_step = np.random.choice(test_X.shape[1], 1)
print(f'Conditioning on the step {conditioning_step} - X: {test_X[0, conditioning_step]}, '
      f'Y: {test_Y[0, conditioning_step]}, G: {test_gamma[0, conditioning_step]}')

observation = np.concatenate((test_X[0, conditioning_step], test_gamma[0, conditioning_step],
                              test_Y[0, conditioning_step])).reshape(1, 1, d_x+d_gamma+d_y)
target_X_gamma = np.concatenate((test_X[0].reshape(1, time_len, d_x), test_gamma[0].reshape(1, time_len, d_gamma)),
                                axis=2)
