...
data_path = "data/1_person_1_obs/combined_all/"
novel_data_path = "data/1_person_1_obs/novel/"

output_path = f'output/{str(int(time.time()))}/'
...
(X, Y, gamma) = sample((X, Y, gamma), num=20)
(v_X, v_Y, v_gamma) = sample((v_X, v_Y, v_gamma), num=5)
# (v_X, v_Y, v_gamma) = sample((X, Y, gamma), num=1)
# (v_X, v_Y, v_gamma) = (X, Y, gamma)
...
def get_train_sample():
    ...
    # FIXME: permutation? choice is easier to understand
    perm = np.random.permutation(time_len)
    observation[0, :n, :d_x] = X[d, perm[:n]]
    ...
    if plot:  # We highly recommend that you customize your own plot function, but you can use this function as default
        if final:
            for i in range(d_y):  # for every feature in Y vector we are plotting training data and its prediction
                ...
                plt.plot(X[-1, :, 0][::-1], novel_Y[0, :, i], color='red')
                plt.plot(X[-1, :, 0][::-1], predicted_Y[:, i], color='black')
                plt.errorbar(X[-1, :, 0][::-1], predicted_Y[:, i], yerr=predicted_std[:, i], color='black', alpha=0.4)
